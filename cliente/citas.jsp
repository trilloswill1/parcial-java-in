...
if (errorConexion != null) {
    ...
    try {
        String sql =
            ...
            "i.razon_social, i.telefono AS inmo_telefono " +
            "FROM cita c " +
        ...
        while (rs.next()) {
            ...
            String telefonoInmo = rs.getString("inmo_telefono");
            citas.add(new String[] { idCita, fechaHora, hora, estado, notas,
                                     idPropiedad, titulo, ciudad, razon, telefonoInmo });
        }
    } catch (SQLException ex) {
        errorCitas = "Ocurrió un error al consultar tus citas. Inténtalo más tarde.";
    } finally {
...
<div class="alert alert-success" role="alert">Tu cita fue agendada correctamente. Quedó en estado PENDIENTE hasta que la inmobiliaria la confirme.</div>
<% } %>
<% if ("1".equals(request.getParameter("cancelada"))) { %>
<div class="alert alert-info" role="alert">Tu cita fue cancelada correctamente.</div>
<% } %>

<form method="get" action="<%= ctx %>/cliente/citas.jsp" class="row g-2 align-items-center mb-3">
    <div class="col-auto">
    ...
    <div class="col-auto">
        ...
        <% if (!filtroEstado.isEmpty()) { %>
        <a href="<%= ctx %>/cliente/citas.jsp" class="btn btn-outline-secondary">Limpiar</a>
        <% } %>
    </div>
</form>
...
<div class="alert alert-info" role="alert">No tienes citas<% if (!filtroEstado.isEmpty()) { %> con estado <%= escapar(filtroEstado) %><% } %>. Puedes agendar una desde el detalle de cualquier propiedad.</div>
<% } else { %>
<div class="table-responsive">
    <table class="table table-striped table-hover align-middle">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    <a href="<%= ctx %>/cliente/detalle-propiedad.jsp?id=<%= c[5] %>"><%= escapar(c[6]) %></a>
                </td>
                <td><%= escapar(c[7]) %></td>
                <td><%= escapar(c[1]) %></td>
                <td><%= escapar(c[2]) %></td>
                ...
                <td>
                    <span class="d-block"><%= escapar(c[8]) %></span>
                    <% if (c[9] != null && !c[9].isEmpty()) { %>
                    <span class="text-muted small">Tel: <%= escapar(c[9]) %></span>
                    <% } %>
                </td>
                <td>
                    <a href="<%= ctx %>/cliente/detalle-cita.jsp?id=<%= c[0] %>" class="btn btn-sm btn-outline-primary">Ver detalle</a>
                </td>
            </tr>
            <% } %>
        </tbody>
