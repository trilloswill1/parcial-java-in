...
String tipoNombre = null, ciudadNombre = null;
...
if (idPropiedad <= 0) {
    ...
    ResultSet rs = null;
    try {
        // Filtra por la inmobiliaria del usuario autenticado: no se puede ver de otra inmobiliaria
        ...
        if (encontrada) {
            ...
            errorDetalle = "Propiedad no encontrada o no autorizada.";
        }
...
<a href="<%= ctx %>/inmobiliaria/propiedades.jsp" class="btn btn-outline-secondary">Volver a mis propiedades</a>
<% } else if (encontrada) { %>
...
<p class="text-muted"><%= escapar(tipoNombre) %> &middot; <%= escapar(ciudadNombre) %></p>
...
<p><%= descripcion != null && !descripcion.trim().isEmpty() ? escapar(descripcion) : "Sin descripción registrada." %></p>

<table class="table table-sm table-borderless w-auto">
    <tbody>
        <tr><td class="text-muted">Dirección</td><td><%= escapar(direccion) %></td></tr>
        <tr><td class="text-muted">Matrícula inmobiliaria</td><td><%= escapar(matricula) %></td></tr>
...
<div class="row g-4 mt-1">
    <div class="col-12 col-lg-6">
        ...
        <% if (imagenes.isEmpty()) { %>
        <p class="text-muted">No se registraron imágenes.</p>
        <% } else { %>
...
<div class="mt-4 d-flex gap-2">
    <a href="<%= ctx %>/inmobiliaria/editar-propiedad.jsp?id=<%= idPropiedad %>" class="btn btn-secondary">Editar</a>
    <a href="<%= ctx %>/inmobiliaria/eliminar-propiedad.jsp?id=<%= idPropiedad %>" class="btn btn-danger">Eliminar</a>
